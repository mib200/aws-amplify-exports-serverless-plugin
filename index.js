...
class ServerlessAmplifyPlugin {
    ...
    getJavaScriptConfiguration(resources, fileDetails) {
        ...
        if (fileDetails.hasOwnProperty('appClient')) {
            const userPool = resources.find(r => r.ResourceType === 'AWS::Cognito::UserPool' && r.LogicalResourceId === fileDetails.userPool);
            if (typeof userPool !== 'undefined') {
                config.aws_cognito_domain = userPool.metadata.UserPool.Domain + ".auth." + this.provider.getRegion() + ".amazoncognito.com";
            }
